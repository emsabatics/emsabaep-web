          ...
          <div class="col-12">
              <div class="card">
                  <div class="card-header">
                      <h3 class="card-title p-2"><i class="fas fa-file-contract mr-3"></i> POA REFORMADO</h3>
                  ...
                  <!-- /.card-header -->
                  <div class="card-body table-responsive p-4" id="divRefPoa">
                    <table class="table datatables" id="tablaRefPOA">
                      ...
                      <tbody>
                        ...
                        <tr id="Tr{{$loop->index}}">
                          ...
                          <td>{{$item->year}}</td>
                          <td>
                            @if ($item->tipo_sel=='general')
                                EMSABA EP
                            @else
                              @foreach ($direccion as $d)
                              @if ($item->id_area == $d->id)
                                  {{$d->nombre}}
                              @endif
                              @endforeach
                            @endif
                          ...
                          <td class="project-actions text-right">
                            <a class="btn btn-primary btn-sm mt-2 mr-3" href="javascript:void(0)" onclick="viewopenRefPOA({{$item->id}})">
                              <i class="fas fa-folder mr-3"></i>
                              Ver
                            </a>
                            <a class="btn btn-info btn-sm mt-2 mr-3" href="javascript:void(0)" onclick="interfaceupdateRefPOA({{$item->id}})">
                              <i class="far fa-edit mr-3"></i>
                              Actualizar
                            </a>
                            @if ($item->estado=='1')
                            <a class="btn btn-secondary btn-sm mt-2 mr-3" href="javascript:void(0)" onclick="inactivarRefPOA({{$item->id}}, {{$loop->index}})">
                              <i class="fas fa-eye-slash mr-3"></i>
                              Inactivar
                            </a>
                            @else
                            <a class="btn btn-secondary btn-sm mt-2 mr-3" href="javascript:void(0)" onclick="activarRefPOA({{$item->id}}, {{$loop->index}})">
