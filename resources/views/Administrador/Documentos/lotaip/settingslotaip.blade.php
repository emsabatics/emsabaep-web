        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="cardsRowTitle">
                            <div class="cardsection">
                              <h3 class="card-title p-2"><i class="fas fa-list-ul mr-3"></i> Literales LOTAIP</h3>
                            </div>
                            <div class="cardsection">
                    ...
                    <!-- /.card-header -->
                    <div class="card-body table-responsive p-4" id="divTablaItemLotaip">
                      <table class="table table-hover text-nowrap table-head-fixed" id="tablaListadoItemLotaip">
                        <thead>
                          <tr style="pointer-events:none;">
                            <th style="width: 10%;text-align: center;">N°</th>
                            <th style="width: 15%;text-align: center;">Artículo</th>
                        ...
                        <tbody>
                          @if (count($itemlotaip) > 0)
                          @foreach ($itemlotaip as $item)
                              <tr id="Tr{{$loop->index}}">
                                <td style="text-align: center;">{{$loop->iteration}}</td>
                                <td style="text-align: center;">{{$item->articulo}}</td>
                                <td class="celdaAsignado">{{$item->literal}}.- {{$item->descripcion}}</td>
                                ...
                                <td style="text-align: center;">
                                  <div class="dropdown show">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                      <a class="dropdown-item" href="javascript:void(0)" onclick="editarItemLiteral({{$item->id}})">Editar</a>
                                      @if ($item->estado=='1')
                                      <a class="dropdown-item" href="javascript:void(0)" onclick="eliminarItemLiteral({{$item->id}}, {{$loop->index}})">Inactivar</a>
                                      @else
                                      <a class="dropdown-item" href="javascript:void(0)" onclick="activarItemLiteral({{$item->id}}, {{$loop->index}})">Activar</a>
                                      @endif
                                    </div>
                                  </div>
                                </td>
...
<div class="modal fade" id="modalAggItLot" tabindex="-1" role="dialog" aria-labelledby="modalArchivosTitle"
aria-hidden="true" data-backdrop="static" data-keyboard="false" data-bs-focus="false">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Agregar Literal LOTAIP</h4>
                ...
                <div class="row">
                    <div class="col-lg-12">
                      <div class="form-group mb-3">
                        <label for="selArtLotaip">Artículo:</label>
                        <select name="selArtLotaip" id="selArtLotaip" class="form-control select2">
                          <optgroup label="Seleccione una Red Social">
                            <option value="0">-Seleccione una opción-</option>
                            @foreach ($artlotaip as $art)
                      ...
                      <div class="form-group mb-3">
                          <label for="inputLiteral">Literal:</label>
                          <input type="text" id="inputLiteral" class="form-control" placeholder="Literal" autocomplete="off">
                      </div>
                      <div class="form-group mb-3">
            ...
            <div class="modal-footer">
                ...
                <button id="btnsaveitlo" type="button" class="btn btn-primary mb-2" onclick="guardarRegistroItLot()">Guardar</button>
            </div>
        </div>
        <!-- /.modal-content -->
    </div>
...
<div class="modal fade" id="modalEditItLot" tabindex="-1" role="dialog" aria-labelledby="modalArchivosTitle"
aria-hidden="true" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Editar Literal LOTAIP</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div class="row">
          <div class="col-lg-12">
            <input type="hidden" name="iditemlotaip" id="iditemlotaip">
            <div class="form-group mb-3 noevent">
              <label for="selArtLotaipEdit">Artículo:</label>
              <select name="selArtLotaipEdit" id="selArtLotaipEdit" class="form-control select2">
                <optgroup label="Seleccione una Red Social">
                  <option value="0">-Seleccione una opción-</option>
                  @foreach ($artlotaip as $art)
                  <option value="{{$art->id}}">{{$art->descripcion}}</option> 
            ...
            <div class="form-group mb-3">
              <label for="inputLiteralE">Literal:</label>
              <input type="text" id="inputLiteralE" class="form-control" placeholder="Literal" autocomplete="off">
            </div>
            <div class="form-group mb-3">
              <label for="inputDescpE">Descripción:</label>
              <input type="text" id="inputDescpE" class="form-control" placeholder="Descripción" autocomplete="off">
            </div>
          </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary mb-2" data-dismiss="modal">Cerrar</button>
        <button id="btnupitlo" type="button" class="btn btn-primary mb-2" onclick="actualizarRegistroItLo()">Actualizar</button>
      </div>
    </div>
    <!-- /.modal-content -->
  </div>
