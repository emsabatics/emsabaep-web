          ...
          <div class="col-12">
              <div class="card">
                  ...
                  <div class="card-body table-responsive p-4" id="divLotaip">
                    <table class="table datatables" id="tablaLOTAIP">
                      ...
                      <tbody>
                        @foreach ($lotaip as $item)
                        <tr id="Tr{{$loop->index}}">
                          <td>{{$loop->iteration}}</td>
                          <td>{{$item->year}}</td>
                          <td>{{$item->mes}}</td>
                          <td>{{$item->literal}}.- {{$item->descripcion}}</td>
                          <td>
                            @if ($item->estado=='0')
                          ...
                          <td class="project-actions text-right">
                            <a class="btn btn-primary btn-sm mt-2 mr-3" href="javascript:void(0)" onclick="viewopenLOTAIP({{$item->id}})">
                              <i class="fas fa-folder mr-3"></i>
                              Ver
                            </a>
                            <a class="btn btn-info btn-sm mt-2 mr-3" href="javascript:void(0)" onclick="interfaceupdateLOTAIP({{$item->id}})">
                              <i class="far fa-edit mr-3"></i>
                              Actualizar
                            </a>
                            @if ($item->estado=='1')
                            <a class="btn btn-secondary btn-sm mt-2 mr-3" href="javascript:void(0)" onclick="inactivarLOTAIP({{$item->id}}, {{$loop->index}})">
                              <i class="fas fa-eye-slash mr-3"></i>
                              Inactivar
                            </a>
                            @else
                            <a class="btn btn-secondary btn-sm mt-2 mr-3" href="javascript:void(0)" onclick="activarLOTAIP({{$item->id}}, {{$loop->index}})">
                              <i class="fas fa-eye mr-3"></i>
                              Activar
                            </a>
                            @endif
                            <a class="btn btn-success btn-sm mt-2 mr-3" onclick="downloadLOTAIP({{$item->id}})">
