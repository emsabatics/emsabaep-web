...
<div class="container-fluid contact bg-light py-5">
    <div class="container py-5">
        <div class="mx-auto text-center mb-5" style="max-width: 900px;">
            @foreach ($docmediosv as $i)
            <h5 class="section-title px-3">
                DOCUMENTACIÓN ADMINISTRATIVA - MEDIOS DE VERIFICACIÓN - AÑO {{$i['anio']}}
            </h5>
            @endforeach
        </div>
                    ...
                    @if($i['longitud']>0)
                        ...
                        <div class="card mt-2">
                            ...
                            <div id="collapse-{{$i['anio']}}-{{$loop->index}}" class="collapse" aria-labelledby="heading-{{$i['anio']}}-{{$loop->index}}" data-parent="#accordion">
                                <div class="card-body">
                                    <button type="button" style="color: white;padding: 5px;font-size: 17px;width: 25%;" class="btn btn-primary btn-sm mr-3 btntable" title="Descargar" onclick="downloadMediosV({{$doc['id']}})">
        ...
        <div class="row g-4 bg-light align-items-center mt-2">
            <div class="co-lg-12">
                <div class="btn-group">
                    <button class="btn-p btn-intermediate" onclick="comeback_listyearmediosv()"><i class="fas fa-arrow-left mr-4"></i> Regresar</button>
                </div>
            </div>
        </div>
    </div>
