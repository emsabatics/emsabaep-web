    ...
    <div class="container py-5">
        <div class="mx-auto text-center mb-5" style="max-width: 900px;">
            ...
            <h5 class="section-title px-3">
                RENDICIÓN DE CUENTAS - AÑO {{$i['anio']}}
            </h5>
            @endforeach
        </div>
                ...
                <div id="accordion" class="myaccordion">
                    @if($i['longitudv']>0)
                        <div class="card mt-2">
                            <div class="card-header" id="headingOne">
                                <h2 class="mb-0">
                                    <button class="d-flex align-items-center justify-content-between btn btn-link" data-toggle="collapse" data-target="#collapse-{{$i['anio']}}-video" aria-expanded="true" aria-controls="collapse-{{$i['anio']}}-video">
                                        RENDICIÓN DE CUENTAS {{$i['anio']}} VIDEO
                            ...
                            <div id="collapse-{{$i['anio']}}-video" class="collapse" aria-labelledby="heading-{{$i['anio']}}-video" data-parent="#accordion">
                                <div class="card-body">
                                    <ul class="accordionul">
                                        @if(count($i['archivos_v']) > 0)
                    ...
                    @if($i['longitudm']>0)
                        <div class="card mt-2">
                            <div class="card-header" id="headingOne">
                                <h2 class="mb-0">
                                    <button class="d-flex align-items-center justify-content-between btn btn-link" data-toggle="collapse" data-target="#collapse-{{$i['anio']}}-medio" aria-expanded="true" aria-controls="collapse-{{$i['anio']}}-video">
                                        MEDIOS DE VERIFICACIÓN {{$i['anio']}}
                                    <span class="fa-stack fa-sm">
                                        <i class="fas fa-circle fa-stack-2x"></i>
                                        <i class="fas fa-plus fa-stack-1x fa-inverse"></i>
                                    </span>
                            ...
                            <div id="collapse-{{$i['anio']}}-medio" class="collapse" aria-labelledby="heading-{{$i['anio']}}-medio" data-parent="#accordion">
                                <div class="card-body">
                                    <ul class="accordionul">
                                        @if(count($i['archivos_m']) > 0)
                                            @foreach ($i['archivos_m'] as $f)
                                                <li>
                                                    <p class="nest">{{$f['titulo']}}</p>
                                                    <ul class="inner">
                                                        <div class="options-list">
        ...
        <div class="row g-4 bg-light align-items-center mt-2">
            <div class="co-lg-12">
                <div class="btn-group">
                    <button class="btn-p btn-intermediate" onclick="comeback_listrc()"><i class="fas fa-arrow-left mr-4"></i> Regresar</button>
                </div>
            </div>
        </div>
    </div>
