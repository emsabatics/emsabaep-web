function openmodalAddOptLotaip(){
    $('#modalAggOpcionLot').modal('show');
}

function guardarRegistroOptLot(){
    var token= $('#token').val();
    ...
    if(descripcion==''){
        ...
        var element= document.getElementById('btnsaveoptlo');
        element.setAttribute("disabled", "");
        element.style.pointerEvents = "none";

        var xr = new XMLHttpRequest();
        xr.open('POST', '/registro-opcion-lotaip', true);
            ...
            if(xr.status === 200){
                ...
                if(myArr.resultado==true){
                    ...
                    setTimeout(function(){
                        ...
                        $('#inputDescp').val("");
                        window.location='/options-lotaip';
...
function editarOptLiteral(id){
    $('#idopcionlotaip').val(id);

    var url= "/get-opcion-lotaip/"+id;
    ...
    xr.onload = function(){
        if(xr.status === 200){
            ...
            $(myArr).each(function(i,v){
                $('#inputDescpEOpt').val(v.descripcion);
            });
            setTimeout(() => {
                $('#modalEditOpcionLot').modal('show');
            }, 300);
        }else if(xr.status === 400){
...
function actualizarRegistroOptLo(){
    var token= $('#token').val();
    var id= $('#idopcionlotaip').val();
    var descripcion= $('#inputDescpEOpt').val();
    ...
    if(descripcion==''){
        ...
        var element= document.getElementById('btnupoptlo');
        element.setAttribute("disabled", "");
        element.style.pointerEvents = "none";

        var xr = new XMLHttpRequest();
        xr.open('POST', '/actualizar-opcion-lotaip', true);
        xr.setRequestHeader('X-CSRF-TOKEN', token);
...
function eliminarOptLiteral(id, i){
    var token= $('#token').val();
    var estado="0";
