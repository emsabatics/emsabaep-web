    ...
    $(array).each(function(i,v){
        if(v.observacion=='no_eliminado'){
            ...
            html +="<tr id='Tr"+i +"'>"+
                ...
                "<td>"+
                    "<div class='cell-img'>"+
                        `<img src="/banner-alcaldia-img/${v.imagen}" alt="${v.imagen}">`+
                    "</div>"+
                "</td>"+
                "<td>"+v.imagen+"</td>"+
...
function urladdnewpics(){
    if(totalr<=5){
        window.location='/registro-banner-alcaldia';
    }else if(totalr==6){
        swal('Solo se permite el ingreso de 6 registros','','warning');
    }
}
...
function guardarImagenes(e){
    ...
    if (lengimg == 0) {
        ...
        swal("Solo se permite seleccionar hasta 6 imágenes", "", "warning");
    } else {
        ...
        setTimeout(() => {
            sendUpdatePicsBanner(token, data, "/banner/registro-banner-alcaldia", element);
        }, 900);
        }else{
    ...
    xr.onload = function(){
        if(xr.status === 200){
            ...
            if(myArr.resultado==true){
                ...
                setTimeout(function(){
                    window.location = "/banner-alcaldia";
                },1500);
...
function openModalOrder(){
    ...
    for(var i=0; i<longitudArray; i++){
        ...
        html+="<tr>"+
            ...
            "<td>"+
                "<div class='cell-img'>"+
                    `<img src="/banner-alcaldia-img/${arrayImg[i]}" alt="${arrayImg[i]}">`+
                 "</div>"+
            "</td>"+
            "<td>"+
...
function actualizarOrden(){
    ...
    if(duplicates.length>0){
        ...
        sendOrderBanner(token, JSON.stringify(objeto), '/banner/registro-orden-banner-alcaldia', element);
        }else{
...
function viewopenimg(i){
   // var urlimg= '../../assets/img/banner/'+arrayImg[i];
    ...
    var html= `<img src="/banner-alcaldia-img/${arrayImg[i]}" alt="${arrayImg[i]}">`;
...
function inactivarBanner(id, i) {
    ...
    if(puedeActualizarM(nameInterfaz) === 'si'){
    $.ajax({
      url: "/in-activar-banner-alcaldia",
      type: "POST",
      dataType: "json",
...
function downloadImg(id){
    ...
    var url='/download-banner-alcaldia/'+id;
    //window.open(url, '_blank');
    window.location='/download-banner-alcaldia/'+id;
    }else{
