function showInfoAdministrativo(){
...
function urlregistrardocadministrativo(){
    window.location='/registrar_doc_administrativo';
}

function urlback(){
    window.location='/docadministrativo';
}
...
function getAliasInput(){
    var year= $('#selYearDocAdmin').select2('data')[0].text;
    if(year!='-Seleccione una Opción-'){
        if($('#inputNameDocAdmin').val()!=''){
            var val= $('#inputNameDocAdmin').val();
...
function generarAlias(){
    ...
    if(year!='-Seleccione una Opción-'){
        if($('#inputNameDocAdmin').val()!=''){
            ...
            let cadenasinpoint= minuscula.replaceAll(/[.,/-]/g,"");
            let cadena= cadenasinpoint.replaceAll(" ","_");
            $('#inputAliasFileDocAdmin').val(year+"_"+cadena);
        }else{
            $('#inputNameDocAdmin').focus();
            toastr.info("Debe ingresar el título correspondiente...", "!Aviso!");
        }
    }else{
        $('#selYearDocAdmin').focus();
        toastr.info("Debe elegir Año correspondiente...", "!Aviso!");
        $('#inputAliasFileDocAdmin').val('');
    }
}

function guardarDocAdmin(){
    var token= $('#token').val();

    let fileInput = document.getElementById("file");
    var year = $("#selYearDocAdmin :selected").val();
    var mes = $("#selMes :selected").val();
    var nombredoc= $('#inputNameDocAdmin').val();
    var aliasfile = $("#inputAliasFileDocAdmin").val();
...
function sendNewDocAdministrativo(token, data, url, el){
...
function viewopenDocAdmin(id){
    window.location='/view-docadministrativo/'+utf8_to_b64(id);
}

/* FUNCION PARA ELIMINAR PERMANENTEMENTE Documentación Administrativa */
...
function inactivarDocAdmin(id, i){
    var token=$('#token').val();
    var estado = "0";
        ...
        if (result.value) {
            ...
            success: function (res) {
                if (res.resultado == true) {
                    ...
                    html+="<button type='button' class='btn btn-primary btn-sm mr-3 btntable' title='Ver' onclick='viewopenDocAdmin("+id+")'>"+
                        "<i class='fas fa-folder mr-2'></i>"+
                        "Ver"+
                    "</button>"+
                    "<button type='button' class='btn btn-info btn-sm mr-3 btntable' title='Actualizar' onclick='interfaceupdateDocAdmin("+id+")'>"+
                        "<i class='far fa-edit mr-2'></i>"+
                        "Actualizar"+
                    "</button>";
                    if(estado=="1"){
                        html+="<button type='button' class='btn btn-secondary btn-sm mr-3 btntable' title='Inactivar' onclick='inactivarDocAdmin("+id+", "+i+")'>"+
                            "<i class='fas fa-eye-slash mr-2'></i>"+
                            "Inactivar"+
                        "</button>";
                    }else if(estado=="0"){
                            html+="<button type='button' class='btn btn-secondary btn-sm mr-3 btntable' title='Activar' onclick='activarDocAdmin("+id+", "+i+")'>"+
...
function interfaceupdateDocAdmin(id){
    window.location= '/edit-docadministrativo/'+utf8_to_b64(id);
}

function generarAliasE(){
    //toastr.info("No se permite generar el Alias...", "!Aviso!");
    var year= $('#selYearEditDocAdmin').select2('data')[0].text;
    if(year!='-Seleccione una Opción-'){
...
function eliminarFile(e){
    ...
    if(element.classList.contains('noshow')){
        ...
        isDocAdministrativo= false;
    }
}

function actualizardocadmin(){
    var token= $('#token').val();
...
/* FUNCION QUE ENVIA LOS DATOS AL SERVIDOR PARA EL REGISTRO */
function sendUpdateDocAdministrativo(token, data, url){
    var contentType = "application/x-www-form-urlencoded;charset=utf-8";
    var xr = new XMLHttpRequest();
    xr.open('POST', url, true);
    //xr.setRequestHeader('Content-Type', contentType);
